...
// Handle form submission for adding new maintenance staff
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'], $_POST['phone'], $_POST['email'])) {
    ...
    if ($name === '') {
        ...
        $stmt = $conn->prepare("INSERT INTO maintenance_staff (name, phone, email) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $name, $phone, $email);
        if ($stmt->execute()) {
            $success = 'Maintenance staff added successfully.';
        } else {
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM maintenance_staff WHERE id = ?");
    $stmt->bind_param("i", $id);
    if ($stmt->execute()) {
        $success = 'Maintenance staff deleted successfully.';
    } else {
...
$result = $conn->query("SELECT * FROM maintenance_staff ORDER BY name ASC");
$staffs = [];
if ($result) {
...
<head>
    ...
    <title>Maintenance Staff - Gym Supervision System</title>
...
<body class="min-h-screen flex flex-col">
    ...
    <main class="flex-grow p-6 max-w-4xl mx-auto">
        <h1 class="text-3xl font-bold mb-6">Maintenance Staff</h1>
        ...
        <form method="POST" action="maintenance_staff.php" class="mb-6 space-y-4 max-w-md">
            <div>
                ...
                <?php foreach ($staffs as $staff): ?>
                    <tr class="border border-gray-700 hover:bg-gray-700">
                        ...
                        <td class="p-3 border border-gray-700">
                            <a href="maintenance_staff.php?delete=<?php echo $staff['id']; ?>" onclick="return confirm('Are you sure you want to delete this staff member?');" class="text-red-500 hover:underline">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
                <?php if (empty($staffs)): ?>
                    <tr>
                        <td colspan="5" class="p-3 text-center text-gray-400">No maintenance staff found.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
