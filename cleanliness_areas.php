...
// Handle form submission for adding new area
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['area_name'])) {
    $area_name = trim($_POST['area_name']);
    if ($area_name === '') {
        $error = 'Area name cannot be empty.';
    } else {
        $stmt = $conn->prepare("INSERT INTO cleanliness_areas (area_name) VALUES (?)");
        $stmt->bind_param("s", $area_name);
        if ($stmt->execute()) {
            $success = 'Cleanliness area added successfully.';
        } else {
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM cleanliness_areas WHERE id = ?");
    $stmt->bind_param("i", $id);
    if ($stmt->execute()) {
        $success = 'Cleanliness area deleted successfully.';
    } else {
...
// Fetch all areas
$result = $conn->query("SELECT * FROM cleanliness_areas ORDER BY area_name ASC");
$areas = [];
if ($result) {
...
<head>
    ...
    <title>Cleanliness Areas - Gym Supervision System</title>
...
<body class="min-h-screen flex flex-col">
    ...
    <main class="flex-grow p-6 max-w-4xl mx-auto">
        <h1 class="text-3xl font-bold mb-6">Cleanliness Areas</h1>
        ...
        <form method="POST" action="cleanliness_areas.php" class="mb-6 flex gap-4">
            <input type="text" name="area_name" placeholder="New area name" required class="flex-grow p-3 rounded bg-gray-800 border border-gray-700 focus:outline-none focus:ring-2 focus:ring-white" />
                ...
                <?php foreach ($areas as $area): ?>
                    <tr class="border border-gray-700 hover:bg-gray-700">
                        ...
                        <td class="p-3 border border-gray-700">
                            <a href="cleanliness_areas.php?delete=<?php echo $area['id']; ?>" onclick="return confirm('Are you sure you want to delete this area?');" class="text-red-500 hover:underline">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
                <?php if (empty($areas)): ?>
                    <tr>
                        <td colspan="3" class="p-3 text-center text-gray-400">No cleanliness areas found.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
