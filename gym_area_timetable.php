...
// Fetch gym areas, staff, and shifts for dropdowns
$areas = [];
$staffs = [];
$shifts = [];

$area_result = $conn->query("SELECT id, area_name FROM gym_areas ORDER BY area_name ASC");
if ($area_result) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['area_id'], $_POST['staff_id'], $_POST['shift_id'], $_POST['timetable_date'])) {
    $area_id = intval($_POST['area_id']);
    $staff_id = intval($_POST['staff_id']);
    $shift_id = intval($_POST['shift_id']);
    $timetable_date = $_POST['timetable_date'];

    if ($area_id <= 0 || $staff_id <= 0 || $shift_id <= 0 || $timetable_date === '') {
        $error = 'All fields are required.';
    } else {
        $stmt = $conn->prepare("INSERT INTO gym_area_timetable (area_id, staff_id, shift_id, timetable_date) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("iiis", $area_id, $staff_id, $shift_id, $timetable_date);
        if ($stmt->execute()) {
            $success = 'Gym area timetable entry added successfully.';
        } else {
...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM gym_area_timetable WHERE id = ?");
    $stmt->bind_param("i", $id);
    if ($stmt->execute()) {
        $success = 'Gym area timetable entry deleted successfully.';
    } else {
...
$sql = "SELECT gat.id, ga.area_name, cs.shift_name, cs.start_time, cs.end_time, cst.name AS staff_name, gat.timetable_date
        FROM gym_area_timetable gat
        JOIN gym_areas ga ON gat.area_id = ga.id
        JOIN cleanliness_shifts cs ON gat.shift_id = cs.id
        JOIN cleaning_staff cst ON gat.staff_id = cst.id
        ORDER BY gat.timetable_date DESC, cs.start_time ASC";
...
<head>
    ...
    <title>Gym Area Timetable - Gym Supervision System</title>
...
<body class="min-h-screen flex flex-col">
    ...
    <main class="flex-grow p-6 max-w-6xl mx-auto">
        <h1 class="text-3xl font-bold mb-6">Gym Area Timetable</h1>
        ...
        <form method="POST" action="gym_area_timetable.php" class="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div>
                <label for="area_id" class="block mb-2 font-semibold">Gym Area</label>
            ...
            <div>
                <label for="timetable_date" class="block mb-2 font-semibold">Date</label>
                <input type="date" id="timetable_date" name="timetable_date" required class="w-full p-3 rounded bg-gray-800 border border-gray-700 focus:outline-none focus:ring-2 focus:ring-white" />
            </div>
        ...
        <table class="w-full text-left border-collapse border border-gray-700">
            <thead>
                <tr class="bg-gray-800">
                    ...
                    <th class="p-3 border border-gray-700">Gym Area</th>
            ...
            <tbody>
                <?php foreach ($timetables as $entry): ?>
                    <tr class="border border-gray-700 hover:bg-gray-700">
                        ...
                        <td class="p-3 border border-gray-700"><?php echo htmlspecialchars($entry['timetable_date']); ?></td>
                        <td class="p-3 border border-gray-700">
                            <a href="gym_area_timetable.php?delete=<?php echo $entry['id']; ?>" onclick="return confirm('Are you sure you want to delete this entry?');" class="text-red-500 hover:underline">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
